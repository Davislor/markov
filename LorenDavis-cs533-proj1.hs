{- LorenDavis-cs533-proj1.hs		Code for CS 533, Mini-Project 1.
 -
...
module MDP (policyIterate, policyIterate1) where
...
policyIterate :: MarkovDP ->
                 Double ->
                 (UArray Int Double, UArray Int Action, [String])
{- Parameters:
...
policyIterate (n, m, r, t) gamma = (v, pi, translatePIUpdates updates )
  where (v, pi, updates ) = policyIterate1 (n, m, r, t)
                                           gamma
                                           (initialPolicy n)
                                           []

policyIterate1 :: MarkovDP ->
                  Double ->
                  UArray Int Action ->
                  [PIUpdate] ->
                  (UArray Int Double, UArray Int Action, [PIUpdate])
{- Parameters:
 - process 	= The Markov decision process to solve, in the form
 -		  (|S|,|A|,R,T)
 - gamma 	= The discount parameter, between 0 and 1.
 ...
 -}
policyIterate1 (n, m, r, t) gamma pi updates | diff == [] =
  (v, pi', updates')
                                               | otherwise =
  policyIterate1 (n, m, r, t) gamma pi' updates'
  where
    states :: [State]
    states = [0..n-1]
    actions :: [Action]
...
{- A helper function that compares the expected value of actions a1 and a2
 ...
 -}
    argmax s0 a1 a2 = let
                        u1 = sum . map (\s' -> t!(s0,a1,s') * v!s') $ states
                        u2 = sum . map (\s' -> t!(s0,a2,s') * v!s') $ states
                      in compare u1 u2
    diff :: PIUpdate
...
showUpdates :: (UArray Int Double, UArray Int Action, [String]) ->
               IO ()
